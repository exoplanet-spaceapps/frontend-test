...
const ThreeScene = () => {
  ...
  useEffect(() => {
    ...
    camera.position.set(0, 0, 80); // Camera positioned to show full sphere on left side
    camera.lookAt(0, 0, 0);
    ...
    controls.minDistance = 30; // Slightly larger than sphere radius (20)
    ...
    const handleClick = (event) => {
      ...
      if (intersects.length > 0) {
        ...
        if (starData && starData.tid) {
          ...
          const cameraPosition = direction.multiplyScalar(80); // Position camera outside sphere
    ...
    const handleFlyToStar = (event) => {
      ...
      if (starData && starData.ra !== undefined && starData.dec !== undefined) {
        ...
        import('../../three/starRenderer').then(({ raDec2Cartesian }) => {
          ...
          const direction = starPosition.clone().normalize();
          const cameraPosition = direction.multiplyScalar(80);
