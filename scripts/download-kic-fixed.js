/**
 * FIXED KIC Bulk Download and Merge
 * Correctly parses pipe-delimited KIC catalog format
 */
...
/**
 * Step 3: Parse KIC catalog (PIPE-DELIMITED FORMAT)
 */
async function parseKICCatalog() {
  return new Promise((resolve, reject) => {
    console.log('🔍 Step 3: Parsing KIC catalog (pipe-delimited format)...\n');
    ...
    rl.on('line', (line) => {
      ...
      if (isHeader) {
        ...
        console.log('\nKIC Format: PIPE-DELIMITED (|)');
        console.log('Column 0: kic_ra');
        console.log('Column 1: kic_dec');
        console.log('Column 14: kic_kepmag');
        console.log('Column 15: kic_kepler_id\n');
        isHeader = false;
        return;
      }
      ...
      try {
        ...
        if (parts.length >= 16) {
          const ra = parseFloat(parts[0]);      // Column 0: kic_ra
          const dec = parseFloat(parts[1]);     // Column 1: kic_dec
          const kepmag = parseFloat(parts[14]); // Column 14: kic_kepmag
          const kepid = parseInt(parts[15]);    // Column 15: kic_kepler_id
          ...
          if (!isNaN(kepid) && !isNaN(ra) && !isNaN(dec)) {
            ...
            if (validCount <= 5) {
              console.log(`Entry ${validCount}: kepid=${kepid}, ra=${ra}, dec=${dec}, mag=${kepmag}`);
            }
          }
        }
      } catch (e) {
...
function mergeKICData(kicMap) {
  ...
  data.forEach((record, index) => {
    if (record.catalog === 'KIC' && record.kepid) {
      ...
      if (coords) {
        ...
        // Debug first 5 updates
        if (updated <= 5) {
          console.log(`Updated record ${index}: kepid=${record.kepid}, ra=${coords.ra}, dec=${coords.dec}`);
        }
      } else {
        notFound++;
      }
  ...
  console.log(`\n✅ Merge complete!`);
  console.log(`KIC records in dataset: ${kicRecords}`);
  ...
  return { updated, notFound, total: data.length, kicRecords };
}
...
async function main() {
  ...
  console.log('\n🚀 KIC BULK DOWNLOAD AND MERGE (FIXED VERSION)\n');
  console.log('='.repeat(60));
  console.log('');
